function [watermarked_image, original_watermark, extracted_watermark, encrypted_watermark] = dwt_svd(watermark_logo, cover_image, block_size, attack,param)
    %% Embedding
    ...
    key = randperm(numel(x));
    K = permuteCipher(x, key);
    ...
    key = randperm(numel(K));
    x = permuteDecipher(K, key);
